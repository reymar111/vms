<template>
    <AuthenticatedLayout>
        <Head title="Settings / Academic Year" />
        ...
        <div class="py-1">
            <div class="mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900 flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0 ">
                    <h3 class="text-2xl font-bold ">Settings - Academic Year</h3>
                ...
                <div class="px-6 pb-6 overflow-x-auto bg-white shadow-md rounded-lg mt-0">
                    ...
                    <div v-if="is_updated" id="alert-border-1" class="flex items-center p-4 mb-4 text-yellow-800 border-t-4 border-yellow-300 bg-yellow-50 " role="alert">
                        <svg class="shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                        </svg>
                        <div class="ms-3 text-sm font-medium">
                    ...
                    <div v-if="is_deleted" id="alert-border-1" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 " role="alert">
                        <svg class="shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                        </svg>
                        <div class="ms-3 text-sm font-medium">
                    ...
                    <table class="min-w-full border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Year</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Semester</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="border-b" v-for="(item, index) in filteredAcademicYears" :key="index">
                                <td class="px-4 py-2">{{ index + 1 }}</td>
                                <td class="px-4 py-2">{{ item.code }}</td>
                                <td class="px-4 py-2">{{ item.description }}</td>
                                <td class="px-4 py-2">{{ item.year }}</td>
                                <td class="px-4 py-2">{{ item.semester != null ? item.semester.name : '' }}</td>
                ...
                <div class="relative bg-white rounded-lg shadow-sm ">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                        <h3 class="text-xl font-semibold text-gray-900 ">
                            {{ form.is_editing ? 'Edit' : 'Create' }} Academic Year
                        </h3>
                    ...
                    <div class="p-4 md:p-5 space-y-4">
                        ...
                        <div>
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">Year</label>
                            <input v-model="form.year" type="number" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required />
                        </div>

                        <div>
                            <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Semester</label>
                            <select v-model="form.semester_id" id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option v-for="(item, index) in semesters" :key="index" :value="item.id">{{ item.name }}</option>
                            </select>
                        </div>
...
export default {
    props: ['academic_years', 'semesters'],
    components: {
        AuthenticatedLayout,
        Link,
        Head
    ...
    computed: {
        filteredAcademicYears() {
            if (!this.search) return this.academic_years;

            return this.academic_years.filter(acad => {
                return Object.values(acad).some(value =>
    ...
    methods: {
        ...
        editItem(item) {
            ...
            this.form.semester_id = item.semester_id
            this.form.active = true
        ...
        save() {
            this.form.post('/academic_year/store', {
                onSuccess: () => {
                    this.closeForm()
        ...
        update() {
            this.form.patch('/academic_year/update/'+this.form.id, {
                onSuccess: () => {
                    this.closeForm()
        ...
        deleteItem() {
            this.form.delete('/academic_year/destroy/'+this.delete_form.id, {
                onSuccess: () => {
